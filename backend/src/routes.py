...
def userinfo():
    ...
    if "userid" not in req.keys():
        ...
        if result is not None:
            ...
            res["strengths"] = ", ".join(result.clifton.split(" "))
            res["interests"] = result.interests.split(" ") if len(result.interests) > 0 else []
            res["favourites"] = result.favourites.split(" ") if len(result.favourites) > 0 else []
            res = json.dumps(res)
